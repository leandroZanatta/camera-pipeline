...
class CallbackFrameData(ctypes.Structure):
    _fields_ = [
        ...
        (
            "data_buffer_size",
            ctypes.c_size_t * 4,
        ),  # size_t data_buffer_size[4]; <<< ADICIONADO
    ]
...
def _load_c_library(lib_path):
    ...
    if not os.path.exists(lib_path):
        print(
            f"ERRO FATAL: Biblioteca C não encontrada em {lib_path}."
            f" Compile a biblioteca C primeiro (cd ../c_src/build && make)",
            file=sys.stderr,
        )
        return None
    try:
...
def _define_c_functions(c_lib):
    ...
    try:
        ...
        print(
            "[C Interface] Assinaturas das funções C definidas."
        )  # Mensagem simplificada
        return True

    except AttributeError as e:
        print(
            f"ERRO FATAL: Função não encontrada na biblioteca C: {e}. "
            f"Verifique a compilação e exportação da biblioteca.",
            file=sys.stderr,
        )
        return False
    except Exception as e:
...
if not IS_INTERFACE_READY:
    print(
        "AVISO: Interface da biblioteca C não está pronta. Algumas funcionalidades podem falhar.",
        file=sys.stderr,
    )
